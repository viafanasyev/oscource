...
__all__ += ["save", "setup_breakpoint", "stop_breakpoint", "call_on_line", "stop_on_line", "add_gdb_command", "add_breakpoint", "get_symbol_address", "call_on_monitor_prompt"]
...
def get_symbol_address(s, symfile="obj/kern/kernel.sym"):
    addrs = [int(sym[:16], 16) for sym in open(symfile)
            if sym[19:].strip() == s]
...
def call_on_monitor_prompt(callback):
    """Returns a monitor that calls 'callback' when QEMU prints a JOS monitor prompt."""
