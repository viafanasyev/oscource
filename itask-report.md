...
Поддерживаются debug entry для глобальных переменных, определённые при помощи `DW_AT_specification`.
В случае GCC, это позволяет находить глобальные переменные, объявление и определение которых находятся в разных файлах.
